...
class ErShouFang5i5jSpider(scrapy.Spider):
    ...
    def _parse_house_item(self, response: HtmlResponse, selector: Selector):
        fang = Esf5i5jItem()
        fang['url'] = response.urljoin(str(selector.xpath('div[2]/h3/a/@href').extract_first()).strip())
        if fang['url'] is None:
            return None
        fang['title'] = str(selector.xpath('div[2]/h3/a/text()').extract_first()).strip()
        # 甘泉宜川 宜川一村 · 内环至中环 · 距中潭路地铁站-4口1373米（步行）
        location_info = str(selector.xpath('div[2]/div[1]/p[2]').xpath('string(.)').extract_first()).split('·')
        fang['xiaoqu'] = str(selector.xpath('div[2]/div[1]/p[2]/a[1]/text()').extract_first()).strip()
        fang['xiaoqu_url'] = response.urljoin(str(selector.xpath('div[2]/div[1]/p[2]/a[1]/@href').extract_first()))
        fang['bankuai'] = str(selector.xpath('div[2]/div[1]/p[2]/text()').extract_first()).strip()
        ...
        house_info = str(selector.xpath('div[2]/div[1]/p[1]').xpath('string(.)').extract_first()).split('·')
        ...
        follow_info = str(selector.xpath('div[2]/div[1]/p[3]').xpath('string(.)').extract_first()).split('·')
